...
//        List<DictItemInfo> dictItemList = DB.findList(DictItemInfo.class, querySql);
        String sql = "select id, name, sys_dict_id, value, code,  order_num  from sys_dict_item order by sys_dict_id, order_num";
        if (DB.getDefault().getConfig().getInnerType().equalsIgnoreCase("h2")) {
            sql = "select id, name, sys_dict_id, `value`, code,  order_num  from sys_dict_item order by sys_dict_id, order_num";
        }
        List<SysDictItemRet> dictItemRetList = DB.findList(SysDictItemRet.class, sql);
